...
async def fetch_pubmed_info(session, disease, semaphore):
...
async def scrape_pubmed_diseases(diseases):
    ...
    async with ClientSession(connector=TCPConnector(limit=10, force_close=True)) as session:
        tasks = {disease: fetch_pubmed_info(session, disease, semaphore) for disease in diseases}
